	...
	go func() {
		...
		for {
			...
			func() {
				...
				if _, err := r.sequencer.consumeEntries(cctx, DefaultIntegrationSizeLimit, r.consumeEntries, false); err != nil {
...
func (s *Storage) init(ctx context.Context) error {
	...
	if err != nil {
		if errors.Is(err, gcs.ErrObjectNotExist) {
			...
			defer c()
			if _, err := s.sequencer.consumeEntries(cctx, DefaultIntegrationSizeLimit, s.consumeEntries, true); err != nil {
...
// consumeEntries incorporates the provided entries into the log starting at fromSeq.
func (s *Storage) consumeEntries(ctx context.Context, fromSeq uint64, entries []storage.SequencedEntry) ([]byte, error) {
	var newRoot []byte
